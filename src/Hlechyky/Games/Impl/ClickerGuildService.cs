    ...
    public static object? HouseSnapshot(string nick, string? json)
    {
        ...
        if (guild?["shelf"] is JsonArray shelf)
            foreach (var g in shelf.OfType<JsonObject>())
            {
                ...
                var q = Int(g["quality"]);
                // Якість до 4: з дев'ятого оновлення буває й розкішний виріб (§B1.2).
                if (Clicker.WareOf(ware) is null || q is < 1 or > 4 || (style.Length > 0 && Clicker.Styles.All(x => x.Key != style))) continue;
