...
/// <summary>
/// Мафія на 3–12 душ. Уся сіль у тому, що обговорення йде в балачці столу (там же говорить Глек-ведучий, а мертві
/// й глядачі мовчать — <see cref="TalkBlock"/>), а сама гра тримає лише те, чого в чаті не зробиш: таємні ролі,
/// нічні дії наосліп і чесний підрахунок голосів. Партія
...
public sealed class Mafia : Game
{
    ...
    /// <summary>
    /// Хто мовчить у балачці столу. Поки партія йде, говорять лише живі гравці: мертві бачать усі ролі (spec §Фази,
    /// п. 6) — раніше це трималось на чесному слові, бо суперечка йшла в загальних Балачках, а тепер її тримає стіл.
    /// Глядач ролей не знає, але й суфлером за чужим столом не буде. У лобі й після партії балакають усі.
    /// </summary>
    public override string? TalkBlock(int? seat)
    {
        if (_phase is MafiaPhase.Lobby or MafiaPhase.Done) return null;
        if (seat is not { } s || !_roles.ContainsKey(s)) return "Поки йде партія, глядачі мовчать";
