...
| `ChatBefore` | `beforeId: number`, `log: boolean` | `ChatMessage[]` | 60 повідомлень, старіших за `beforeId` (від старших до новіших): Балачки або Журнал (`log`). Так гортання вгору підвантажує історію. |
| `SendChat` / `SendReply` | `text` / `text, replyTo` | `string\|null` | Загальні Балачки. Відмова — лише тому, хто писав: зокрема захист від флуду («Не так швидко — дай іншим слово вставити» понад 5 повідомлень за 5 с, «Це вже написано» — те саме двічі поспіль за 30 с; команди на повтор не перевіряються). |
...
| `tableHistory` | одному з'єднанню | `{ id: string, lines: TableLine[] }` — уся розмова столу (до 100 реплік), щойно з'єднання підписалось на стіл |
