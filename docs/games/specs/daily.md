...
- **Ключ кімнати.** Ключ має бути `daily:<game>:<day>:<nick_key>` — саме з нього беруться гра і день.
  Ключ іншої форми запишеться як звичайний соло-результат і в панель «Щоденного глека» не потрапить.

Нагорода: гра кличе `Ctx.Award(seat, DailyReward, "daily:<game>")`, а WP1 підставляє ref
`daily:<game>:<day>:<nick_key>` — раз на день на головоломку, як і сказано вище. Ачівки `wordle-2`,
